...
function WorkerHome({ navigation }) {
  ...
  useEffect(() => {
    async function handleScrapData() {
      try {
        const res = await axios.get("http://192.168.52.229:3000/api/v1/user");
        ...
        const resWorker = await axios.get(
          `http://192.168.52.229:3000/api/v1/worker/${globalId}`
        );
  ...
  async function handleStatus(id, status) {
    try {
      ...
      await axios.put(`http://192.168.52.229:3000/api/v1/worker/${globalId}`, {
      ...
      await axios.put(
        `http://192.168.52.229:3000/api/v1/user/worker/${user.phone}`,
        { order: updatedOrders }
      );
