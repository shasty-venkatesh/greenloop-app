...
function CompanyHome({ navigation }) {
  ...
  const fetchGroupOrder = useCallback(
    debounce(async () => {
      try {
        const res = await axios.get("http://192.168.52.229:3000/api/v1/worker");
    ...
    async (order) => {
      ...
      try {
        const resorder = await axios.get(
          `http://192.168.52.229:3000/api/v1/company/${globalId}`
        );
        ...
        await axios.put(`http://192.168.52.229:3000/api/v1/company/${globalId}`, {
          order: [...remainingOrder, confirmOrder],
        });

        const resworker = await axios.get(
          "http://192.168.52.229:3000/api/v1/worker"
        );
        ...
        const resworkercompany = await axios.put(
          `http://192.168.52.229:3000/api/v1/user/worker/company/${sanitizedPhone}`,
          {
        ...
        const response = await axios.get(
          `http://192.168.52.229:3000/api/v1/worker/${id}`
        );
        ...
        await axios.put(`http://192.168.52.229:3000/api/v1/worker/${id}`, {
